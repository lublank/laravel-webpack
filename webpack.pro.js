...
//资源路径
let assetsPath = path.resolve(__dirname, './public');
/*
 |--------------------------------------------------------------------------
 | Mix Initialization
 |--------------------------------------------------------------------------
...
module.exports.entry = {
    app: "./js/app.js",     //common js (公共js)
    vendor: [               //dependency library (依赖包js)
        'jquery',
        'lodash'
    ],
...
// 输出路径
// 与webpack.dev.js 不同的是生产环境打包的资源输出到特定地方，便于做CDN，以及做代码压缩加密等处理
...
module.exports.module = {
    rules: [
        ...
        // 打包svg sprite
        {
            // 匹配./resources/assets/images目录下的icon-*.svg
            test: /icon-.+\.svg$/,
            use: [{
                loader: 'svg-sprite-loader'
            }, {
                loader: 'svgo-loader',
                // svgo 配置，将 <svg> 中一些无用的信息过滤去除，精简结构
                options: {
                    plugins: [
                        { removeTitle: true },
...

        {
            ...
            exclude: /icon-.+\.svg$/,   // 这步很重要，除去上面的icon-svg
            loader: 'file-loader',
            options: {
...
module.exports.resolve = {
    ...
    extensions: ['*', '.js', '.jsx'],
    modules: ['node_modules', 'sprites']

    //模块别名定义，方便后续直接引用别名，无须多写长长的地址
    // alias: {
    //     'vue$': 'vue/dist/vue.common.js'//后续直接 require('vue$') 即可
    // }
};
...
module.exports.devServer = {
    contentBase: path.resolve(__dirname, './resources/assets'),  // 热替换
...
module.exports.plugins = (module.exports.plugins || []).concat([
    ...
    new SpritesmithPlugin({
        src: {
            cwd: path.resolve(__dirname, 'resources/assets/images'),
            glob: '**/icon-*.png'
        },
        target: {
            image: path.resolve(__dirname, 'sprites/icon-sprite.png'),
            css: path.resolve(__dirname, 'sprites/icon-sprite.scss')
        },
        apiOptions: {
            cssImageRef: "~icon-sprite.png"
        }
    })
]);
